...
def affine_backward(dout, cache):
    ...
    x = x.reshape(N,D)
    dx = np.dot(dout,w.T)
    #print("whole shape is",x.shape)
    dx = dx.reshape(whole_shape)
...
def relu_forward(x):
    ...
    # print("before index is",type(out))
    out[index]=0
    # print("after index is",type(out))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    cache = x
...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        cache['eps'] = eps
        cache['x'] = np.copy(x)
        cache['offset'] = x - sample_mean
        cache['scaling'] = 1.0/np.sqrt(sample_var+eps)
        cache['normalized_x'] = np.copy(x_hat)
        out = np.copy(y)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
        ...
        #######################################################################
        y1 = (gamma/(np.sqrt(bn_param['running_var']+eps))) * x
        y2 =  beta - (gamma*bn_param['running_mean'])/(np.sqrt(bn_param['running_var']+eps))
        y = y1 + y2
        out = np.copy(y)
        #######################################################################
        #                          END OF YOUR CODE                           #
...
def batchnorm_backward(dout, cache):
    ...
    ###########################################################################
    m = dout.shape[0]
    partial_x_hat = dout * cache['gamma']
    # print("gamma shape is",cache['gamma'].shape)
    # print("dout shape is",dout.shape)
    # print("partial_x_hart shape is",partial_x_hat.shape)
    partial_var = np.sum(partial_x_hat * cache['offset'] * -0.5 * (cache['scaling']*cache['scaling']*cache['scaling']),axis=0)
    partial_mean = np.sum(partial_x_hat*-1*cache['scaling'],axis=0) + partial_var*(1.0/m)*np.sum(-2*cache['offset'],axis=0)
    dx = partial_x_hat*cache['scaling'] + partial_var *((2.0*cache['offset'])/m) + (1.0/m)*partial_mean
    
    dgamma = np.sum(dout*cache['normalized_x'],axis=0) # shape (D,)
...
def batchnorm_backward_alt(dout, cache):
    ...
    an1_1 = 0.0
    partial_ca = np.zeros(dout.shape)
    for i in range(dout.shape[0]):
        for j in range(dout.shape[1]):
            if j == 0 :
                if i == 0 :
                    partial_ca[i,j] = (1-1.0/N)
                else :
                    partial_ca[i,j] = (-1.0/N)
    print("partical_ca is",partial_ca)
    partial_de = 0.0

    #for k in range(dout.shape[0]):
    #    partial_de += (cache['offset'][k][j])*(partial_ca[k][j])


    pp_ = np.zeros(dout.shape[1])
    #for i in range(pp_.shape[0]):
    #    for j in range(N):
            
    for i in range(dout.shape[0]):
        for j in range(dout.shape[1]):  
            if j == 0 : 
                partial_de = cache['offset'][0,j]
            else :
                partial_de = 0.0
            up = (cache['offset'][i,j]*cache['scaling'][j]**3)
            an1_1 += cache['gamma'][j] * dout[i,j] * ((partial_ca[i,j]*cache['scaling'][j]) - ((up/(N))*partial_de))


    multi_ = cache['scaling'][0] * multi
    print("multi_ shape",multi_.shape)
    print("scaling[0]",cache['scaling'][0])
    answer = dout[:,0] * multi_
    print(answer.shape)
    print("answer", an1_1)
    # partial_hat_to_x = 
    #dx = dout * cache['gamma'] * (-(1.0/m)) * np.linalg.norm(cache['scaling']*cache['scaling'])
    #dx_re = dx[range(m),:]
    print("dx shape is",dx.shape)
    print("normalized_x shape is",cache['normalized_x'].shape)
    dgamma = np.sum(dout*cache['normalized_x'],axis=0)
...
def dropout_backward(dout, cache):
    ...
    if mode == 'train':
        ...
        #######################################################################
        #print("mask shape is",cache['mask'].shape)
        dx = dout * mask
        #######################################################################
        #                          END OF YOUR CODE                           #
        #######################################################################
